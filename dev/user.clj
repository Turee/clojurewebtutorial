(ns user
  (:require
    ...
    [clojurewebtutorial.config-reader :as conf]
...
(defn migratus-config []
  (-> (system) :migratus :config))


(defn create-migration [migration-name]
  (mig/create (migratus-config) migration-name))

(defn migrate []
  (mig/migrate (migratus-config)))
